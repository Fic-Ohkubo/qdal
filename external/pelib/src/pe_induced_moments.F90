...
subroutine iterative_solver(Mkinds, Fks)
    ...
    use pe_blas_interfaces, only: nrm2, spmv, dot
    ...
    integer :: DIIS_size, DIIS_info
    integer, parameter :: DIIS_max_size = 25
    integer, dimension(1) :: DIIS_iter
    real(dp), dimension(:,:), allocatable :: DIIS_previous_induced_moments, DIIS_residuals
    real(dp), dimension(:,:), allocatable :: DIIS_Bmat
    real(dp), dimension(:), allocatable :: DIIS_weights
    integer, dimension(:), allocatable  :: DIIS_ipiv
    ...
    do n = 1, size(Mkinds, 2)
        ...
        DIIS_previous_induced_moments = 0.0_dp
        DIIS_residuals = 0.0_dp
        DIIS_iter = 1
        ...
        do
            ...
            if (iter > 2) then
                ...
                DIIS_Bmat(1:DIIS_size,1:DIIS_size) = 0.0_dp
                DIIS_Bmat(1:DIIS_size,DIIS_size) = -1.0_dp
                DIIS_Bmat(DIIS_size,1:DIIS_size) = -1.0_dp
                DIIS_Bmat(DIIS_size,DIIS_size) = 0.0_dp
                do i = 1, DIIS_size-1
                    do j = 1, DIIS_size-1
                ...
                DIIS_weights(1:DIIS_size-1) = 0.0_dp
                DIIS_weights(DIIS_size) = -1.0_dp
                ! Solve B x mu = rhs
            ...
            do i = site_start, site_finish
                ...
                do j = 1, nsites
                    ...
                    call spmv(T2, Mkinds(m:m+2,n), Ftmp(l:l+2), 'L', 1.0_dp, 1.0_dp)
                    m = m + 3
                end do
                l = l + 3
            end do
