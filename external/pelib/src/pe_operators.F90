...
subroutine get_fock(denmats, fckmats, energies)
    ...
    if (fock .or. energy) then
        ...
        if (myid == master) then
            do i = 1, ndens
                Epe(i) = sum(Ees(1:2,i)) + sum(Epol(:,i)) + sum(Elj(:))
            end do
...
subroutine print_info()
 ...
 20 format(/7x,a)
    ...
    write(luout,'(/7x,a17,9x,f20.12)') 'Total PE energy: ', sum(Ees(1:2,1)) + sum(Epol(:,1)) + sum(Elj(:))
...
subroutine es_multipoles(denmats, fckmats)
    ...
    use pe_multipole_tools, only: multipole_interaction_energy, &
                                  prefactors, Tk_tensor

    real(dp), dimension(:), intent(in) :: denmats
    real(dp), dimension(:), intent(inout), optional :: fckmats

    logical :: lexist, exclude
    integer :: lu
    integer :: site, ncomps
    integer :: i, j, k
    real(dp) :: tmp_energy
    real(dp), dimension(3) :: Rsm, Rij
    ...
    if (lexist .and. fock .and. ((scf_cycle > 1) .or. pe_restart)) then
        ...
        if ((myid == master) .and. (pe_skipmul .eqv. .false.)) then
