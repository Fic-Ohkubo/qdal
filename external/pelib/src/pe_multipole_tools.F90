...
subroutine multipole_interaction_energy(E, Mi, Rij, Mj)
    ...
    real(dp), dimension(:), intent(in) :: Mi
    real(dp), dimension(:), intent(in) :: Mj
    ...
    integer :: a_x, a_y, a_z
    integer :: b_x, b_y, b_z
    integer :: a_norm, a_fact, a_idx
    integer :: b_norm, b_fact, b_idx
    real(dp) :: taylor

    E = 0.0_dp

    ! the order of the first multipole
    a_order = int(0.5_dp * (sqrt(1.0_dp + 8.0_dp * real(size(Mi), dp)) - 1.0_dp)) - 1

    ! the order of the second multipole
    b_order = int(0.5_dp * (sqrt(1.0_dp + 8.0_dp * real(size(Mj), dp)) - 1.0_dp)) - 1
