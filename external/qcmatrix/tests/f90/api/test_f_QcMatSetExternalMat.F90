...
!!
!!  This file tests the function QcMatSetExternalMat_f().
!!
!!  2014-03-13, Bin Gao:
!!  * first version
...
#define QCMATRIX_F_TEST_SRC "tests/f90/api/test_f_QcMatSetExternalMat.F90"

    !% \brief tests the function QcMatSetExternalMat_f()
    !  \author Bin Gao
    !  \date 2014-03-13
    !  \param[QcMat:type]{inout} A the matrix
    !% \param[integer]{in} io_log IO of the logfile
    subroutine test_f_QcMatSetExternalMat(A, io_log)
        ...
        type(QcMat) B                                        !test matrix
        type(QcMat) C                                        !duplication of matrix A
        ...
        if (ierr==QSUCCESS) then
            if (.not. assembled) then
                ...
                write(io_log,100) "QcMatSetExternalMat_f() will not be tested ..."
                return
            end if
        else
            call QErrorExit(io_log, __LINE__, QCMATRIX_F_TEST_SRC)
        end if
        ! creates the test matrix B
        ...
        ierr = QcMatDuplicate_f(A, COPY_PATTERN_AND_VALUE, C)
        call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
        ...
        call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
        write(io_log,100) "QcMatGetDimBlock(A) passed ..."
        ...
        ! gets the external matrix
        ierr = QcMatGetExternalMat_f(A=A, A_ext=A_ext)
        call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
        write(io_log,100) "QcMatGetExternalMat_f(A) passed ..."
        ! sets the external matrix
        ierr = QcMatSetExternalMat_f(A=B, A_ext=A_ext)
        call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
        write(io_log,100) "QcMatSetExternalMat_f(B) passed ..."
        ...
        call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
        write(io_log,100) "QcMatGetExternalMat_f(A) passed ..."
        ! sets the external matrix (real part)
        call Matrix_IsAssembled(A_ext, assembled)
        if (assembled) then
            ierr = QcMatSetExternalMat_f(A=B,                &
                                         data_type=QREALMAT, &
                                         A_ext=A_ext)
            call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
            write(io_log,100) "QcMatSetExternalMat_f(B) passed ..."
        ...
        call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
        write(io_log,100) "QcMatGetExternalMat_f(A) passed ..."
        ! sets the external matrix (imaginar part)
        call Matrix_IsAssembled(A_ext, assembled)
        if (assembled) then
            ierr = QcMatSetExternalMat_f(A=B,                &
                                         data_type=QIMAGMAT, &
        ...
        do iblk = 1, num_blocks
            if (data_type(iblk)/=QNULLMAT) then
                ...
                call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
                write(io_log,100) "QcMatGetExternalMat_f(A) passed ..."
                ! sets the external complex matrix for this block
                ierr = QcMatSetExternalMat_f(A=B,                               &
                                             idx_block_row=idx_block_row(iblk), &
                                             idx_block_col=idx_block_col(iblk), &
                                             A_ext=A_ext)
...
#endif
        ! tests if we really set the matrix B by the external matrix A_ext
...
#if defined(QCMATRIX_ENABLE_VIEW)
            ierr = QcMatWrite_f(A, "QcMatSetExternalMat_A", ASCII_VIEW)
            call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
            ierr = QcMatWrite_f(B, "QcMatSetExternalMat_B", ASCII_VIEW)
            call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
#endif
            call QErrorExit(io_log, __LINE__, QCMATRIX_F_TEST_SRC)
        end if
        ...
        call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
        ! the context of A should not be destroyed
        ierr = QcMatIsEqual_f(A, C, cf_values, is_equal)
        call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
        if (is_equal) then
            write(io_log,100) "QcMatIsEqual_f(A, C) passed ..."
        else
            ! dumps results to check
#if defined(QCMATRIX_ENABLE_VIEW)
            ierr = QcMatWrite_f(A, "QcMatSetExternalMat_A", ASCII_VIEW)
            call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
            ierr = QcMatWrite_f(C, "QcMatSetExternalMat_C", ASCII_VIEW)
            call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
#endif
            call QErrorExit(io_log, __LINE__, QCMATRIX_F_TEST_SRC)
        end if
        ! frees the space taken by the matrix C
        ierr = QcMatDestroy_f(C)
        call QErrorCheckCode(io_log, ierr, __LINE__, QCMATRIX_F_TEST_SRC)
        return
100     format("test_f_QcMatSetExternalMat>> ",A)
    end subroutine test_f_QcMatSetExternalMat
