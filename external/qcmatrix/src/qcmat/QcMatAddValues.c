/* QcMatrix: an abstract matrix library
   ...
   This file implements the function QcMatAddValues().
...
#endif
        printf("QcMatAddValues>> input index of block row %"QINT_FMT"\n",
               idx_block_row);
...
#endif
        printf("QcMatAddValues>> input index of block column %"QINT_FMT"\n",
               idx_block_col);
...
#if defined(QCMATRIX_ZERO_BASED)
    err_code = CmplxMatAddValues(&A->blocks[idx_block_row][idx_block_col],
                                 idx_first_row,
                                 num_row_add,
                                 idx_first_col,
                                 num_col_add,
                                 ...
                                 values_imag);
#else
    err_code = CmplxMatAddValues(&A->blocks[idx_block_row-1][idx_block_col-1],
                                 idx_first_row,
                                 num_row_add,
                                 idx_first_col,
                                 num_col_add,
                                 ...
                                 values_imag);
#endif
    QErrorCheckCode(err_code, FILE_AND_LINE, "calling CmplxMatAddValues");
    return QSUCCESS;
}
